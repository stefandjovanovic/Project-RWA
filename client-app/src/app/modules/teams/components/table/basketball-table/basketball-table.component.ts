...
@Component({
  ...
  styleUrl: './basketball-table.component.css'
})
export class BasketballTableComponent implements OnInit, AfterViewInit, OnDestroy{
  tableData: TableData[] = [];
  ...
  ngOnInit() {
    this.storeSubscription = this.store.select(TableSelectors.selectBasketballData).subscribe(data => {
      if(data.length > 0){
        this.tableData = data;
