...
@Component({
  ...
  styleUrl: './football-table.component.css'
})
export class FootballTableComponent implements OnInit, OnDestroy, AfterViewInit{
  tableData: TableData[] = [];
  ...
  ngOnInit() {
    this.storeSubscription = this.store.select(TableSelectors.selectFootballData).subscribe(data => {
      if(data.length > 0){
        this.tableData = data;
